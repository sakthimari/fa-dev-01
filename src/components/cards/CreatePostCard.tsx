...
const CreatePostCard = () => {
  const guests = [avatar1, avatar2, avatar3, avatar4, avatar5, avatar6]
  ...
  const handleCreatePost = async () => {
    if (!postContent.trim() && selectedFiles.length === 0) {
      setPostError('Please add some content or select images to post.')
      return
    }
    ...
    try {
      ...
      setPostError(error instanceof Error ? error.message : 'Failed to create post. Please try again.')
    } finally {
  ...
  return (
    <>
          ...
          <div className="avatar avatar-xs me-2">
            <span role="button">
              <img 
                ...
                src={profile?.profilePhotoUrl || avatar3} 
                alt="Profile" 
                onError={(e) => {
                  // Fallback to default avatar if image fails to load
                  const target = e.target as HTMLImageElement
                  if (target.src !== avatar3) {
                    target.src = avatar3
                  }
                }}
              />
      ...
      <Modal
        ...
        <ModalBody>
          {postError && (
            <Alert variant="danger" dismissible onClose={() => setPostError(null)}>
              {postError}
            </Alert>
          )}
        ...
        <ModalFooter>
          ...
          <button 
            ...
            disabled={isCreatingPost || (!postContent.trim() && selectedFiles.length === 0)}
          >
